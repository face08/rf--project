package com.youbt.rpc
{
	...
	import com.youbt.events.RemoteOperationEvent;
	...
	*  远端返回信息处理成功
	*/
	[Event(name="remote_operation_succesed", type="com.youbt.events.RemoteOperationEvent")]
	
	/**
	* 远端服务出错，失败 
	*/
	[Event(name="remote_operation_fault", type="com.youbt.events.RemoteOperationEvent")]
	
	/**
	* 远端服务返回数据格式错误或者数据处理出错，失败 
	*/
	[Event(name="remote_operation_data_failed", type="com.youbt.events.RemoteOperationEvent")]
	...
	*/	
	...
	public class RemoteToken extends AsyncToken
	{
		...
		public function RemoteToken(CurrentOperation:Object=null,target:IEventDispatcher=null)
		{
			...
			RFTraceSummary(getOperationName()+' been Called!');
		}
		...
		public function dispatchSuccessed(msg:String='',data:* =''):Boolean
		{
			...
			RFTraceSummary(getOperationName()+' Successed!',msg);
		...
		public function dispatchFailed(msg:String='',data:*=''):Boolean
		{
			...
			RFTraceWarn(getOperationName()+' Failed!',msg,data);
			currentOperation=null
		...
		public function dispatchFault(msg:String='',data:*=''):Boolean
		{
			...
			RFTraceError(getOperationName()+' Fault!',msg,data);
			currentOperation=null
			return dispatchEvent(event);
		}
		...
		public function dispatchProgress(progressEvent:ProgressEvent):Boolean
		{
			...
			RFTraceSummary(getOperationName()+' ProgressEvent',
